...
instnames = ['foces', 'lick', 'elodie']    # list of instrument names. Can be whatever you like (no spaces) but should match 'tel' column in the input file.
...
anybasis_params['per1'] = radvel.Parameter(value=4.230785)      # period of 1st planet
anybasis_params['tc1'] = radvel.Parameter(value=2450001.51)     # time of inferior conjunction of 1st planet
anybasis_params['e1'] = radvel.Parameter(value=0.013)          # eccentricity of 1st planet
anybasis_params['w1'] = radvel.Parameter(value=2.0*np.pi*58.0/360.0)      # argument of periastron of the star's orbit for 1st planet
anybasis_params['k1'] = radvel.Parameter(value=55.94)          # velocity semi-amplitude for 1st planet
...
anybasis_params['gamma_foces'] = radvel.Parameter(value=0.0)       # velocity zero-point for foces
anybasis_params['gamma_lick'] = radvel.Parameter(value=0.0)       # velocity zero-point for lick
anybasis_params['gamma_elodie'] = radvel.Parameter(value=0.0)       # velocity zero-point for elodie

anybasis_params['jit_foces'] = radvel.Parameter(value=2.6)        # jitter for foces
anybasis_params['jit_lick'] = radvel.Parameter(value=2.6)        # jitter for lick
anybasis_params['jit_elodie'] = radvel.Parameter(value=2.6)        # jitter for elodie
